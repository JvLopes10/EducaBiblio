...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'Cadastrar') {
        ...
        if ($_FILES['FotoLivro']['error'] === UPLOAD_ERR_OK) {
            ...
            // Movendo o arquivo para o diretório de destino, se um arquivo foi enviado
            if (!empty($nomeArquivo) && is_uploaded_file($arquivoTemp)) {
            ...
            try {
                ...
                // Define o valor padrão para o campo da imagem se nenhum arquivo foi enviado
                $conteudoImagem = !empty($nomeArquivo) ? file_get_contents($caminhoArquivo) : null;

                // Vincular os parâmetros da consulta, incluindo o campo de imagem
